...
# 🆕 Credentials pour réauthentification automatique
...
def reauthenticate_roblox():
    """Réauthentifie l'utilisateur Roblox et récupère un nouveau cookie"""
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if new_cookie:
                logger.info(f"✅ Nouveau cookie généré: {new_cookie[:30]}...")
                return new_cookie
            else:
            ...
            try:
                data = resp.json()
                if data.get("errors"):
                    error = data["errors"][0].get("message", "Unknown")
                    logger.error(f"❌ Login échoué: {error}")
            except:
                pass
            return None
        else:
            logger.error(f"❌ Login échoué HTTP {resp.status_code}")
            return None
...
def verify_cookie_validity(cookie, group_id):
    """Vérifie si le cookie est valide en testant une requête simple"""
    try:
        resp = requests.get(
        ...
        if resp.status_code == 200:
            logger.info("✅ Cookie valide")
            return True
        elif resp.status_code in (401, 403):
            logger.warning(f"⚠️ Cookie invalide (HTTP {resp.status_code})")
            return False
        else:
            logger.warning(f"⚠️ Statut inattendu: {resp.status_code}")
            return True  # On essaie quand même
            
    except Exception as e:
        logger.error(f"❌ Erreur vérification cookie: {e}")
        return True  # On essaie quand même en cas de timeout


def refresh_cookie_if_needed():
    """Refresh le cookie si le délai est écoulé"""
    global cookie_last_refresh
    
    with cookie_lock:
        now = datetime.now()
        time_since_refresh = (now - cookie_last_refresh).total_seconds()
        
        if time_since_refresh > COOKIE_REFRESH_INTERVAL:
            logger.info(f"🔄 Délai de refresh écoulé ({time_since_refresh}s), réauthentification...")
            ...
            if new_cookie:
                ...
                logger.warning("⚠️ Réauthentification échouée, on continue avec l'ancien cookie")
...
# ============================================================================
# ROBLOX PAYOUT — RobloxPayout Class (AMÉLIORÉ)
# ============================================================================

class RobloxPayout:
    """Gère les transferts Robux avec support Chef challenge + 2FA TOTP + Anti-AutomatedTampering + Refresh Cookie"""
    ...
    def _set_csrf(self):
        """Récupère un nouveau token CSRF + vérifie le cookie"""
        # Vérifier d'abord la validité du cookie
        if not verify_cookie_validity(self.roblosecurity, self.group_id):
            logger.warning("[CSRF] 🔄 Tentative de réauthentification...")
            new_cookie = reauthenticate_roblox()
            if new_cookie:
    ...
    def payout(self, user_id, amount, retry_count=0, max_retries=3):
        ...
        if challenge_type == "chef":
            ...
            if next_type == "":
                ...
                time.sleep(DELAY_BEFORE_FINAL)  # ⏳ Délai avant final
                ...
                time.sleep(DELAY_AFTER_2FA)  # ⏳ Délai après 2FA
                complete_meta = {
                ...
                if cont2_type == "blocksession":
                    ...
                    if retry_count < max_retries:
                        logger.info(f"[Chef+2FA] ⏳ Attente {DELAY_BLOCKSESSION}s avant retry...")
                ...
                if retry_count < max_retries:
                    logger.info(f"[Chef] ⏳ Attente {DELAY_BLOCKSESSION}s avant retry...")
                    time.sleep(DELAY_BLOCKSESSION)
                    self._set_csrf()
                    return self.payout(user_id, amount, retry_count + 1, max_retries)
                else:
        ...
        # ── TWOSTEPVERIFICATION DIRECT ──────────────────────────────────────
            ...
            if cont2_type == "blocksession":
                ...
                if retry_count < max_retries:
                    logger.info(f"[2FA] ⏳ Attente {DELAY_BLOCKSESSION}s avant retry...")
                    time.sleep(DELAY_BLOCKSESSION)
                    self._set_csrf()
                    return self.payout(user_id, amount, retry_count + 1, max_retries)
                else:
            ...
            if retry_count < max_retries:
                logger.info(f"[Payout] ⏳ Attente {DELAY_BLOCKSESSION}s avant retry...")
                time.sleep(DELAY_BLOCKSESSION)
                self._set_csrf()
                return self.payout(user_id, amount, retry_count + 1, max_retries)
            else:
...
# ============================================================================
# ADMIN DASHBOARD (identique à l'original)
# ============================================================================
